//CdnPath=http://ajax.aspnetcdn.com/ajax/4.5.1/1/WebTEST.js
var __wpm = null;
function Point(x, y) {
    this.x = x;
    this.y = y;
...
function Zone_Dispose() {
    for (var i = 0; i < this.webTEST.length; i++) {
        this.webTEST[i].Dispose();
    }
...
function Zone_AddWebPart(webPartElement, webPartTitleElement, allowZoneChange) {
    var webPart = null;
    var zoneIndex = this.webTEST.length;
    ...
    this.webTEST[zoneIndex] = webPart;
    return webPart;
}
...
function Zone_GetWebPartIndex(location) {
    ...
    var webTEST = this.webTEST;
    var webTESTCount = webTEST.length;
    for (var i = 0; i < webTESTCount; i++) {
        var webPart = webTEST[i];
        if (vertical) {
