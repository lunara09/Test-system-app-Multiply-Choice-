...
namespace WebTests
{
    ...
    [Serializable]
    public partial class PageQuestion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                var item = new ListItem { Value = q.QUESTIONID.ToString(), Text = string.Concat("Вопрос ", q.QUESTIONID.ToString()) };
        ...
        protected void Answer(object sender, EventArgs e)
        {
            ...
            if (Session["ss"] == null)
            {
                ...
                //правильные ответы
                var q = obj.QUESTION.Where(l => l.TESTID == testId);
                List<bool> flags = new List<bool>();
                foreach (var item in q)
                {
                    var t = obj.QA.Where(a => a.QUESTIONID == QuestId).First().CORRECTANSWER.Value;
                    flags.Add(t);
                }
                ...
                foreach (var flag in flags)
                {
                    if (flag == true)
                    {
                        (FindControl("chk" + i.ToString()) as CheckBox).BackColor = System.Drawing.Color.YellowGreen;
                    }
                    else
                    {
                        (FindControl("chk" + i.ToString()) as CheckBox).BackColor = System.Drawing.Color.SeaGreen;
                    }
                    if (flag != chks[i])
                    {
                        counter++;
        ...
        protected void Next(object sender, EventArgs e)
        {
            ...
            questId++;
            string url = "PageQuestion.aspx?testId=" + testId + "&questid=" + questId;
            Response.Redirect(url);
        }
